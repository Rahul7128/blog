...
with open('C:\\Users\\Rahul Sangwan\\OneDrive\Desktop\\website designing using flask\\config.json', 'r') as c:
...
def uploader():
    if ('user' in session and session['user'] == params['admin_usrname']):
...
def dashboard():
    ...
    if request.method=="POST":
        ...
        if (username==params['admin_usrname'] and userpass==params['admin_password']):
